...
import io.nuls.contract.token.interfaces.INRC1363;
...
public class CrossLockedToken extends Ownable implements Contract, Token, INRC1363 {
    ...
    public CrossLockedToken(@Required String name, @Required String symbol, @Required BigInteger initialAmount,
    ...
    public void loadCrossTokenSystemContract() {
        CROSS_TOKEN_SYSTEM_CONTRACT = new Address(
                (String) Utils.invokeExternalCmd("sc_getCrossTokenSystemContract", null));
    }
    ...
    private void checkBatchData(String[] targets, long[] values) {
        require(targets.length == values.length,
                "The number of addresses and amounts for batch transfer should be the same");
    }
    ...
    @Override
    public boolean transferAndCall(Address to, BigInteger value, String data) {
        if (!transfer(to, value)) {
            revert("NRC1363TransferFailed");
        }
        require(to.isContract(), "NRC1363EOAReceiver");
        String[][] args = new String[][] {
    ...
    @Override
    public boolean transferFromAndCall(Address from, Address to, BigInteger value, String data) {
    ...
    @Override
    public boolean approveAndCall(Address spender, BigInteger value, String data) {
        if (!approve(spender, value)) {
            revert("NRC1363ApproveFailed");
        }
        require(spender.isContract(), "NRC1363EOASpender");
        String[][] args = new String[][] {
                new String[] { Msg.sender().toString() },
                new String[] { value.toString() },
                new String[] { data } };
